...
pub struct DualChannelManager {
    pub state_a: ChannelState,
    pub state_b: ChannelState,
...
impl DualChannelManager {
    pub fn new() -> Self {
        Self {
            state_a: ChannelState::Shadow,
            state_b: ChannelState::Active,
            active: ChannelId::A,
            hysteresis: 0,
        }
    }
    pub fn arbitrate(&mut self, a: ChannelHealth, b: ChannelHealth) -> ChannelId {
        ...
        if (sa - sb).abs() > 15 {
            ...
            self.hysteresis = (self.hysteresis + 1).min(3);
            self.active
        }
    }
}
